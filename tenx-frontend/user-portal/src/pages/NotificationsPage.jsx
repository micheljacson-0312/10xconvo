...
export function NotificationsPage() {
  ...
  const markAll = async () => {
    try { await notifApi.markAll(); toast.success('All marked as read'); load() } catch {}
  }
    ...
    <>
      <div className="page-header">
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <h2>Notifications</h2>
          {unread > 0 && (
            <span style={{ background:'var(--accent)',color:'#fff',borderRadius:10,padding:'1px 8px',fontSize:12,fontWeight:700 }}>
              {unread}
            </span>
          )}
        </div>
        <div style={{ display:'flex', gap:8 }}>
          <button className={`btn ${unreadOnly?'btn-primary':'btn-ghost'}`} style={{fontSize:12}}
            onClick={() => setFilter(f=>!f)}>{unreadOnly?'Unread Only':'All'}</button>
          {unread > 0 && (
...
export function NotificationBell() {
  ...
  const markRead = async id => { try { await notifApi.markRead(id); load() } catch {} }
  const markAll  = async ()  => { try { await notifApi.markAll(); load() } catch {} }

  return (
    <div style={{ position:'relative' }}>
      <button onClick={() => setOpen(o=>!o)} style={{
        position:'relative', background:'none', border:'none', cursor:'pointer',
        padding:6, borderRadius:8, color: unread>0 ? 'var(--accent)' : 'var(--muted)',
      }}>
        <Bell size={18}/>
        {unread > 0 && (
          <span style={{
            position:'absolute', top:0, right:0,
            background:'var(--accent2)', color:'#fff', borderRadius:'50%',
        ...
        <>
          ...
          <div style={{
            ...
            <div style={{padding:'12px 16px',borderBottom:'1px solid var(--border)',
              ...
              {unread>0 && <button onClick={markAll} style={{background:'none',border:'none',cursor:'pointer',
            ...
            {items.length === 0 ? (
              ...
              <div key={n.id} onClick={() => { if(!n.isRead) markRead(n.id) }}
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{color:'var(--muted)',fontSize:11,marginTop:3}}>{timeAgo(n.createdOn)}</div>
                </div>
              </div>
            ))}
          </div>
